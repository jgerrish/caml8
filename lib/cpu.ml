...
let execute_instruction (t : t) (instruction : Instruction.t) : next_pc =
  ...
  | Se_vx_nn (vx, nn) ->
    ...
    if Uint8.compare vx' nn = 0 then begin
      Skip
    end else Next
  | Se_vx_vy (vx, vy) ->
    let vx = read_register t vx in
  ...
  | Skp vx ->
    ...
    begin match t.key_state with
      | Some k ->
        if Uint8.compare vx k = 0 then begin
          Skip
        end else
          Next
      | None -> Next
  ...
  | Sub (vx, vy) ->
    let vx' = read_register t vx in
    let vy' = read_register t vy in
    let borrow_flag = if Uint8.compare vx' vy' <> -1 then Uint8.one else Uint8.zero in
  ...
  | Subn (vx, vy) ->
    let vx' = read_register t vx in
    let vy' = read_register t vy in
    let borrow_flag = if Uint8.compare vy' vx' <> -1 then Uint8.one else Uint8.zero in
    write_register t Registers.vf borrow_flag;
  ...
  | Drw (vx, vy, n) ->
    ...
    Registers.set t.registers Registers.vf Uint8.zero;
